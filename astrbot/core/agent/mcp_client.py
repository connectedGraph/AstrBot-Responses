        ...
        if "url" in cfg:
            ...
            if transport_type != "streamable_http":
                ...
                http_client = httpx2.AsyncClient(
                    headers=cfg.get("headers", {}),
                    timeout=httpx2.Timeout(timeout_val),
                )
                ...
                streams_res = await self.exit_stack.enter_async_context(
                    self._streams_context,
                )
                if isinstance(streams_res, (list, tuple)):
                    read_s, write_s = streams_res[0], streams_res[1]
                else:
                    read_s, write_s = streams_res
    ...
    async def call_tool_with_reconnect(
        ...
        async def _call_with_retry():
            ...
            timeout_sec = (
                ...
                else (float(read_timeout_seconds) if read_timeout_seconds is not None else None)
            )
            try:
...
class MCPTool(FunctionTool, Generic[TContext]):
    ...
    def __init__(
        ...
        super().__init__(
            ...
            parameters=_normalize_mcp_input_schema(getattr(mcp_tool, 'input_schema', getattr(mcp_tool, 'inputSchema', {}))),
        )
