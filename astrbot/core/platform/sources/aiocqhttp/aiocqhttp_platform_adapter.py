...
class AiocqhttpAdapter(Platform):
    ...
    async def _convert_handle_notice_event(self, event: Event) -> AstrBotMessage:
        ...
        if event.get("notice_type") == "group_upload" and event.get("file"):
            upload_file = event["file"]
            file_id = str(upload_file.get("id", ""))
            if file_id and event.get("group_id"):
                try:
                    ...
                    if file_url:
                        ...
                        logger.info(
                            "aiocqhttp group_upload 已转换为 File: name=%s group_id=%s",
                            upload_file.get("name", "file"),
                            event.group_id,
                        )
                    else:
                        logger.error("group_upload 获取文件 URL 失败: %s", result)
                except Exception as exc:  # noqa: BLE001
                    logger.exception("group_upload 获取文件 URL 异常: %s", exc)
            ...
            if t == "text":
                ...
                for m in m_group:
                    if m["data"].get("url") and m["data"].get("url").startswith("http"):
                        ...
                        ret = None
                        file_id = m["data"].get("file_id") or m["data"].get("file")
                        # 1. 优先尝试 NapCat 的 get_file 接口（直接调用 QQNT 原生 FileApi，无需 packetBackend）
                        if file_id:
                            try:
                                ...
                                logger.debug(f"get_file 调用失败: {e}，尝试群/私聊链接接口...")

                        # 2. 若 get_file 未成功，回退到原有 get_group_file_url / get_private_file_url
                        if not ret or not (isinstance(ret, dict) and (ret.get("url") or ret.get("file"))):
                            try:
                                if abm.type == MessageType.GROUP_MESSAGE and "file_id" in m["data"]:
                                    ret = await self.bot.call_action(
                                        action="get_group_file_url",
                                        file_id=m["data"]["file_id"],
                                        group_id=event.group_id,
                                    )
                                elif abm.type == MessageType.FRIEND_MESSAGE and "file_id" in m["data"]:
                                    ret = await self.bot.call_action(
                                        action="get_private_file_url",
                                        file_id=m["data"]["file_id"],
                                    )
                            except Exception as e:
                                logger.warning(f"获取文件链接失败: {e}")

                        if ret and isinstance(ret, dict):
                            # 从返回值提取 url 或 本地文件路径
                            file_url = ret.get("url") or ""
                            file_path = ret.get("file") or ret.get("path") or ""
                            if file_path and not file_url.startswith("http"):
                                import os
                            ...
                            if file_url:
                                ...
                                logger.error(f"获取文件失败，返回值无有效路径或 URL: {ret}")
                        else:
                            # 3. 兜底方案：直接使用原始消息段构建 File，防止整个文件段被忽略丢弃
                            raw_file = m["data"].get("file") or m["data"].get("name") or "file"
                            raw_url = m["data"].get("url") or ""
                            a = File(name=str(raw_file), url=str(raw_url))
                            abm.message.append(a)
                            logger.info(f"使用原始消息段构建 File 兜底: name={raw_file}")
