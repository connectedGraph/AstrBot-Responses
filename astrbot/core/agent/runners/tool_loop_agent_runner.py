...
class ToolLoopAgentRunner(BaseAgentRunner[TContext]):
    ...
    def _agent_responses_from_trace_chains(
        ...
        for chain in llm_response.trace_chains:
            if chain is None:
                continue
            yield AgentResponse(
    ...
    async def step(self):
        ...
        async for llm_response in self._iter_llm_responses_with_fallback():
            for trace_response in self._agent_responses_from_trace_chains(llm_response):
                yield trace_response
            llm_response.trace_chains = []
        ...
        for trace_response in self._agent_responses_from_trace_chains(llm_resp):
            yield trace_response
    ...
    async def _handle_function_tools(
        ...
        for func_tool_name, func_tool_args, func_tool_id in zip(
            ...
            func_tool_for_display = None
            if req.func_tool:
                if (
                    self.tool_schema_mode == "skills_like"
                    and self._skill_like_raw_tool_set
                ):
                    func_tool_for_display = self._skill_like_raw_tool_set.get_tool(
                        func_tool_name
                    )
                else:
                    func_tool_for_display = req.func_tool.get_tool(func_tool_name)
